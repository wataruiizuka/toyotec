DROP TABLE IF EXISTS gold.vbi002011;
CREATE TABLE gold.vbi002011 AS
...
nt8014m AS(
    SELECT
        ...
        , t014m.cd_kaisya
        , t014m.no_cyumon
        , COUNT(IF(t014m.kb_nyukkanr = '01',t014m.dd_keijyou,NULL)) AS cnt
...
kt8014m AS(
    SELECT
        ...
        , t001g.cd_kaisya
        , t001g.no_cyumon
        , TRIM(t001g.no_cyumoned) AS no_cyumoned
        , SUM(IF(t014m.dd_keijyou < CAST(FROM_TIMESTAMP(t001g.dd_touroku, 'yyyyMMdd') AS INT),t014m.ki_nyukinur,0)) AS ki_nyukinur1
...
SELECT
    ...
    , t001g.no_cyumon AS 注文ＮＯ
    , TRIM(t001g.no_cyumoned) AS 注文枝番
    , CONCAT(t001g.cd_hansya, t001g.cd_kaisya, t001g.no_cyumon, TRIM(t001g.no_cyumoned)) AS pk_販社会社注文no枝番
    , CONCAT(t001g.cd_hansya, t001g.cd_kaisya, t001g.cd_tenpo) AS 販社会社店舗コード
    , v2001.kj_tentanms AS 店舗略称
    , v2001.zone_name_abbreviation AS エリア統括部
    , v2001.cd_zon AS ゾーンコード
    , v2001.sort_order AS ソート順
    , IF(t001g.dd_touroku IS NOT NULL,'登録済','未登録') AS 登録
    , IF(t001g.dd_nosya IS NOT NULL,'納車済','未納車') AS 納車
    , IF((NVL(t052g.ki_haseirui,0) - NVL(t052g.ki_genknykg,0) - NVL(t052g.ki_sitanykg,0) - NVL(t052g.ki_fuharnyk,0)) = 0,'残高なし','残高あり') AS 売掛残高
    , t014m.kj_syainmei AS 社員名
    , DATEDIFF(FROM_UTC_TIMESTAMP(UTC_TIMESTAMP(), 'JST'), t001g.dd_touroku) AS 売上経過日数
    , CONCAT(REGEXP_REPLACE(t001g.kj_kainmei1, '　+$', ''), REGEXP_REPLACE(t001g.kj_kainmei2, '　+$', '')) AS 買主名
    , CONCAT(REGEXP_REPLACE(t001g.kj_meigime1, '　+$', ''), REGEXP_REPLACE(t001g.kj_meigime2, '　+$', '')) AS 名義人
    , CASE t052g.mj_keiykeit
        ...
        WHEN '' THEN 'その他'
    END AS 契約形態
    , TRIM(CONCAT(t001g.no_cyumon, t001g.no_cyumoned)) AS 注文no_枝番
    , SPLIT_PART(t001g.mj_hantenkt, '-', 1) AS 型式
    , t001g.dd_jucyu AS 受注日
    , t001g.dd_fr AS 振当日
    , t052g.dd_maeuknyu AS 申込金受領日
    , t001g.dd_touroku AS 登録日
    , t001g.dd_haisou AS 配送日
    , t001g.dd_nosya AS 納車日
    , NVL(t052g.dd_minasksy, t052g.dd_kaisyuyo) AS 回収予定日
    , TO_TIMESTAMP(CAST(nt8014m1.dd_keijyou1 AS STRING), 'yyyyMMdd') AS 最終入金日現金
    , TO_TIMESTAMP(CAST(nt8014m1.dd_keijyou2 AS STRING), 'yyyyMMdd') AS 最終入金日割賦
    , TO_TIMESTAMP(CAST(nt8014m1.dd_keijyou3 AS STRING), 'yyyyMMdd') AS 下取完了日
    , t001g.dd_urikzumi AS 完了
    , t052g.ki_genknykg AS 現金入金額
    , t052g.ki_sitanykg AS 下取車入金額
    , t052g.ki_fuharnyk AS 賦払金入金額
    , (NVL(t052g.ki_haseirui,0) - NVL(t052g.ki_genknykg,0) - NVL(t052g.ki_sitanykg,0) - NVL(t052g.ki_fuharnyk,0)) AS 売掛金残高合計
    , t052g.ki_genkhasg AS 現金発生額
    , (NVL(t052g.ki_genkhasg,0) - NVL(t052g.ki_genknykg,0)) AS 現金残高
    , t052g.ki_fuharhsk AS 賦払金発生額
    , (NVL(t052g.ki_fuharhsk,0) - NVL(t052g.ki_fuharnyk,0)) AS 割賦残高
    , t052g.ki_sitahasg AS 下取車発生額
    , (NVL(t052g.ki_sitahasg,0) - NVL(t052g.ki_sitanykg,0)) AS 下取車残高
    , IF (
        t001g.dd_touroku IS NULL OR nt8014m1.dd_keijyou1 IS NULL OR TO_TIMESTAMP(CAST(nt8014m1.dd_keijyou1 AS STRING), 'yyyyMMdd') < t001g.dd_touroku,
        t052g.ki_nyuruike,
    ...
    , DATEDIFF(t001g.dd_nosya, t001g.dd_fr) AS '振当-納車'
    , DATEDIFF(t001g.dd_nosya, t001g.dd_touroku) AS '登録-納車'
    , DATEDIFF(t001g.dd_urikzumi, t001g.dd_touroku) AS '登録-完了'
    ...
    , t052g.ki_haseirui AS 発生累計
    , t001g.dt_saisinup AS 更新日時
FROM ai21rep_ve_dx.tbba001g t001g
...
LEFT JOIN ai21rep_ve_dx.tbba051g t051g1
...
LEFT JOIN ai21rep_ve_dx.tbba051g t051g2
...
WHERE isnottrue(
    (t052g.ki_haseirui - t052g.ki_genknykg - t052g.ki_sitanykg - t052g.ki_fuharnyk) = 0
    AND((t001g.dd_touroku IS NOT NULL AND t001g.dd_nosya < DATE '2023-03-01'
        AND (t001g.dd_urikzumi IS NULL OR t001g.dd_urikzumi < DATE '2024-03-01'))
        OR (t001g.dd_nosya IS NULL AND t001g.dd_touroku < DATE '2023-03-01')
        OR t001g.dd_urikzumi < DATE '2024-03-01'
        OR (t001g.dd_touroku IS NULL AND t001g.dd_jucyu < DATE '2020-12-01')
        )
    )
AND (t001g.dd_torotrkk IS NULL
    OR (t052g.ki_haseirui - t052g.ki_genknykg - t052g.ki_sitanykg - t052g.ki_fuharnyk) <> 0)
AND (LEFT(t001g.kb_haraidas, 1) = '1'
    OR (LEFT(t001g.kb_haraidas, 1) = '3'
        AND NOT((t052g.ki_haseirui - t052g.ki_genknykg - t052g.ki_sitanykg - t052g.ki_fuharnyk) = 0
            AND (t052g.ki_genkhasg - t052g.ki_genknykg) = 0
            AND (t052g.ki_fuharhsk - t052g.ki_fuharnyk) = 0
            AND (t052g.ki_sitahasg - t052g.ki_sitanykg) = 0
            )
        )
    )
;
